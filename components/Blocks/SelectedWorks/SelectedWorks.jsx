...
export default function SelectedWorks() {
  ...
  useGSAP(
    () => {
      ...
      mm.add("(max-width: 201px)", () => {
        tl.to(gallery, {
          ease: "none",
        });
      });
      mm.add("(min-width: 202px)", () => {
        tl.to(gallery, {
          x: () => {
            return `-${gallery?.clientWidth - window.innerWidth}`;
          },
      ...
      <div className={styles.xScrollContainer} ref={galleryContainer}>
        <header className={styles.header}>
          ...
          <TextReveal className={styles.description}>
            We&apos;ve played a key role in developing impactful projects.
            Here&apos;s a curated selection showcasing my expertise and the
            achieved results.
          </TextReveal>
          <FancyButton
            theme="button-2"
            link={`mailto:${commonConfig.personal.email}`}
            target={"_blank"}
          >
            Contact
          </FancyButton>
